...
export function PostForm({ initialData, categories }: PostFormProps) {
  ...
  let defaultImageId = FALLBACK_IMAGES[(initialData?.title?.length || 0) % FALLBACK_IMAGES.length];
  ...
  const effectiveImageUrl = imageUrl || `https://images.unsplash.com/photo-${defaultImageId}?w=800&q=80&auto=format`;
    ...
    <form onSubmit={handleSubmit} className="flex flex-col xl:flex-row gap-6 items-start animate-in fade-in duration-500">
      ...
      <div className="flex-1 w-full space-y-6 min-w-0">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm space-y-6">
          ...
          <div className="space-y-2" data-color-mode={mounted && resolvedTheme === 'dark' ? 'dark' : 'light'}>
      ...
      <div className="w-full xl:w-80 shrink-0 space-y-6">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm space-y-6">
          ...
          <div className="space-y-3">
            <label htmlFor="image_url" className="text-sm font-medium">Thumbnail Image URL</label>
            <div className="w-full aspect-video rounded-md overflow-hidden bg-muted border border-border relative">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={effectiveImageUrl} alt="Thumbnail preview" className="w-full h-full object-cover" onError={(e) => (e.currentTarget.style.display = 'none')} onLoad={(e) => (e.currentTarget.style.display = 'block')} />
              {!imageUrl && (
